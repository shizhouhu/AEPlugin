...
//                            GAIN_DISK_ID);
    ...
    PF_ADD_POINT("start", 25, 25, false, START_ID);
	AEFX_CLR_STRUCT(def);
    PF_ADD_POINT("end", 75, 75, false, END_ID);
...
void drawline(int x0, int y0, int x1, int y1, PF_EffectWorld *output, PF_Pixel color)
{
    int dx, dy, p, x, y;
	...
	PF_LayerDef		*output )
{
    ...
    A_long x = FIX_2_FLOAT(params[SKELETON_START]->u.td.x_value),
           y = FIX_2_FLOAT(params[SKELETON_START]->u.td.y_value),
           x1 = FIX_2_FLOAT(params[SKELETON_END]->u.td.x_value),
           y1 = FIX_2_FLOAT(params[SKELETON_END]->u.td.y_value);
    
    PF_Pixel color = params[SKELETON_COLOR]->u.cd.value;
    color.alpha = 255;
    drawline(x, y, x1, y1, output, color);
